    ...
    if layer.type == 'Convolution' or layer.type == 'Deconvolution':
        ...
        node_label = '"%s%s(%s %s)%skernel size: %d%sstride: %d%spad: %d"' %\
                     (layer.name,
                      ...
                      layer.type,
                      separator,
                      1, #layer.pooling_param.kernel_size[0] if len(layer.pooling_param.kernel_size._values) else 1,
                      separator,
                      1, #layer.pooling_param.stride[0] if len(layer.pooling_param.stride._values) else 1,
                      separator,
                      0) #layer.pooling_param.pad[0] if len(layer.pooling_param.pad._values) else 0)
    else:
