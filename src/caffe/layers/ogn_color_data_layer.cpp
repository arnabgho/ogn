...
void OGNColorDataLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    const vector<Blob<Dtype>*>& top)
{
    ...
    else
    {
        ...
        values_shape.push_back(batch_size); values_shape.push_back(3); values_shape.push_back(num_elements);
    }
...
template <typename Dtype>
void OGNColorDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    ...
    for(int bt=0; bt<batch_size; bt++)
    {
        ...
        int counter = 0;
        for(ColorOctree::iterator it=_batch_octrees[bt].begin(); it!=_batch_octrees[bt].end(); it++)
        {  
            unsigned int val=it->second;
            for(int ch=0;ch<3;ch++){
                int top_index = bt * 3 * num_elements + ch*num_elements + counter;
                unsigned char c= (val & ( (1<<8) -1));
                top_values[top_index] = (Dtype)c  ;
                val=val>>8;
            }
...
template <typename Dtype>
void OGNColorDataLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_CLASS(OGNColorDataLayer);
REGISTER_LAYER_CLASS(OGNColorData);
